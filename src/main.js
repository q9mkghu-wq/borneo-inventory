...
<head>
  ...
  <style>
    *{box-sizing:border-box;margin:0;padding:0;}
    ...
    .warn-text{color:#d97706;font-size:13px;margin-top:6px;}
    ...
    .rank-num.bronze{background:#fee2e2;color:#991b1b;}
    ...
    .timeline-date{min-width:90px;color:#6b7280;font-size:13px;}
    ...
    .predict-info{flex:1;}
    .predict-name{font-size:14px;font-weight:500;margin-bottom:3px;}
    .predict-detail{font-size:12px;color:#6b7280;}
    .report-driver{border:1px solid #e5e7eb;border-radius:12px;padding:1rem;margin-bottom:1rem;}
    .report-driver-header{display:flex;align-items:center;justify-content:space-between;margin-bottom:12px;}
    .report-driver-name{font-size:16px;font-weight:500;}
    ...
    .modal{background:#fff;border-radius:12px;padding:1.5rem;width:360px;max-width:92vw;box-shadow:0 8px 32px rgba(0,0,0,0.18);}
    ...
    .step-badge{width:24px;height:24px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:600;flex-shrink:0;margin-top:20px;}
    .step-badge.on{background:#111827;color:#fff;}
    .step-badge.off{background:#f3f4f6;color:#9ca3af;}
    .step-inner{flex:1;}
    .step-inner label{display:block;font-size:13px;color:#6b7280;margin-bottom:5px;font-weight:500;}
    ...
    .opt-tag button:hover{color:#dc2626;}
    .add-row{display:flex;gap:8px;}
    .add-row input{flex:1;font-size:13px;padding:7px 10px;}
    .check-row{display:flex;align-items:center;gap:8px;margin-bottom:8px;}
    .check-row input[type=checkbox]{width:16px;height:16px;flex-shrink:0;}
    ...
    .parts-summary tr.ps-warn td{background:#fef3c7;}
    .parts-summary tr.ps-err td{background:#fee2e2;}
    .qty-badge{display:inline-block;font-size:12px;font-weight:600;padding:2px 10px;border-radius:12px;}
    .qty-badge.ok{background:#d1fae5;color:#065f46;}
    .qty-badge.low{background:#fef3c7;color:#92400e;}
    .qty-badge.empty{background:#fee2e2;color:#991b1b;}
    .prod-parts-item{margin-bottom:10px;padding:10px 12px;background:#f3f4f6;border-radius:8px;}
    .prod-parts-title{font-size:13px;font-weight:600;margin-bottom:6px;}
    .part-line{display:flex;justify-content:space-between;font-size:13px;padding:3px 0;border-bottom:1px solid #e5e7eb;}
    ...
    .print-modal{background:#fff;border-radius:12px;width:100%;max-width:720px;max-height:92vh;overflow-y:auto;box-shadow:0 20px 60px rgba(0,0,0,0.3);}
    ...
    .print-doc .doc-sub{font-size:13px;color:#6b7280;}
    .print-doc .doc-meta{display:flex;gap:20px;margin-bottom:16px;font-size:12px;color:#374151;flex-wrap:wrap;}
    ...
    .print-doc th{background:#f3f4f6;text-align:left;padding:7px 10px;border:1px solid #d1d5db;font-weight:600;font-size:12px;}
    .print-doc td{padding:7px 10px;border:1px solid #d1d5db;vertical-align:middle;}
    .print-doc .need-qty{font-size:16px;font-weight:700;}
    .print-doc .stock-ok{color:#065f46;font-weight:500;}
    .print-doc .stock-low{color:#92400e;font-weight:500;}
    .print-doc .stock-empty{color:#991b1b;font-weight:500;}
    ...
    .print-doc .warning-row td{background:#fff7ed;}
    .print-doc .danger-row td{background:#fef2f2;}
    @media print{
...
<div class="container">
  ...
  <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;flex-wrap:wrap;gap:8px;">
    <div>
      <div style="font-size:18px;font-weight:500;">보루네오 재고 관리</div>
      <div style="font-size:13px;color:#6b7280;margin-top:2px;">가구 배송 부품 재고 시스템</div>
    </div>
    <div style="display:flex;gap:8px;">
      <button class="success" onclick="exportExcel()">엑셀 다운로드</button>
      <button class="secondary" style="font-size:13px;" onclick="location.reload()">새로고침</button>
    </div>
  </div>
  ...
  <div class="sync-bar">
    ...
    <span style="margin-left:auto;font-size:12px;color:#6b7280;" id="syncTime"></span>
  </div>
  ...
  <div class="tabs">
    ...
    <button class="tab" onclick="showTab('관리')">제품/부품/기사 관리</button>
  </div>
  ...
  <div class="section active" id="tab-출고">
    <div class="card">
      ...
      <div style="border-top:1px solid #e5e7eb;margin-top:8px;padding-top:14px;">
        ...
        <div class="step-wrap"><div class="step-badge on" id="sb1">1</div><div class="step-inner"><label>카테고리</label><select id="sc_cat" onchange="step1_cat()"><option value="">-- 선택하세요 --</option></select></div></div>
        <div class="step-wrap" id="sw2" style="display:none;"><div class="step-badge off" id="sb2">2</div><div class="step-inner"><label id="sl2">재질</label><select id="sc_sub" onchange="step2_sub()"><option value="">-- 선택하세요 --</option></select></div></div>
        <div class="step-wrap" id="sw3" style="display:none;"><div class="step-badge off" id="sb3">3</div><div class="step-inner"><label>모델</label><select id="sc_model" onchange="step3_model()"><option value="">-- 선택하세요 --</option></select></div></div>
        <div class="step-wrap" id="sw4" style="display:none;"><div class="step-badge off" id="sb4">4</div><div class="step-inner"><label>사이즈</label><select id="sc_size" onchange="step4_size()"><option value="">-- 선택하세요 --</option></select></div></div>
        <div class="step-wrap" id="sw5" style="display:none;"><div class="step-badge off" id="sb5">5</div><div class="step-inner"><label>색상</label><select id="sc_color" onchange="step5_color()"><option value="">-- 선택하세요 --</option></select></div></div>
        <div class="step-wrap" id="sw6" style="display:none;"><div class="step-badge off" id="sb6">6</div><div class="step-inner"><label>매트리스 포함 여부</label><select id="sc_mattyn" onchange="step6_mattyn()"><option value="">-- 선택하세요 --</option><option value="없음">매트리스 없음</option><option value="있음">매트리스 포함</option></select></div></div>
        <div class="step-wrap" id="sw7" style="display:none;"><div class="step-badge off" id="sb7">7</div><div class="step-inner"><label>매트리스 종류</label><select id="sc_matttype" onchange="step7_matttype()"><option value="">-- 선택하세요 --</option></select></div></div>
        <div class="step-wrap" id="sw8" style="display:none;"><div class="step-badge off" id="sb8">8</div><div class="step-inner"><label>매트리스 사이즈</label><select id="sc_mattsize" onchange="step8_mattsize()"><option value="">-- 선택하세요 --</option></select></div></div>
        <div class="result-sel" id="resultSel" style="display:none;"><div class="result-sel-title" id="rSelTitle"></div><div class="result-sel-path" id="rSelPath"></div><div id="rSelParts"></div></div>
      </div>
      <div style="margin-top:1rem;display:flex;align-items:center;gap:10px;"><label style="font-size:14px;color:#6b7280;">수량</label><input type="number" id="qty" value="1" min="1" max="50" style="width:80px;"><span style="font-size:14px;color:#6b7280;">세트</span></div>
  ...
  <div class="section" id="tab-묶음">
    <div class="card">
      ...
      <div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(120px,1fr));gap:8px;margin-bottom:10px;">
        <div><div style="font-size:12px;color:#6b7280;margin-bottom:4px;">카테고리</div><select id="bc_cat" onchange="bOnCat()" style="width:100%;font-size:13px;"><option value="">선택</option></select></div>
        <div id="bc_sub_w" style="display:none;"><div style="font-size:12px;color:#6b7280;margin-bottom:4px;" id="bc_sub_lbl">재질</div><select id="bc_sub" onchange="bOnSub()" style="width:100%;font-size:13px;"><option value="">선택</option></select></div>
        <div id="bc_model_w" style="display:none;"><div style="font-size:12px;color:#6b7280;margin-bottom:4px;">모델</div><select id="bc_model" onchange="bOnModel()" style="width:100%;font-size:13px;"><option value="">선택</option></select></div>
        <div id="bc_size_w" style="display:none;"><div style="font-size:12px;color:#6b7280;margin-bottom:4px;">사이즈</div><select id="bc_size" onchange="bOnSize()" style="width:100%;font-size:13px;"><option value="">선택</option></select></div>
        <div id="bc_color_w" style="display:none;"><div style="font-size:12px;color:#6b7280;margin-bottom:4px;">색상</div><select id="bc_color" onchange="bOnColor()" style="width:100%;font-size:13px;"><option value="">선택</option></select></div>
      </div>
      <div id="bc_add_row" style="display:none;">
        <div style="font-size:12px;color:#065f46;margin-bottom:8px;font-weight:500;" id="bc_preview_name"></div>
        <div class="row">
  ...
  <div class="section" id="tab-입고">
    ...
    <div class="card"><h3>일괄 입고</h3>
      <div style="font-size:13px;color:#6b7280;margin-bottom:12px;">여러 부품을 한번에 입고 처리합니다.</div>
  ...
  <div class="section" id="tab-관리">
    ...
    <div class="card">
      <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:1rem;flex-wrap:wrap;gap:8px;"><h3 style="margin-bottom:0;">부품 관리</h3><div style="display:flex;gap:8px;"><input type="text" id="searchPartMgmt" placeholder="부품명 검색..." oninput="renderPartMgmt()" style="width:150px;font-size:13px;"><button class="primary" style="font-size:13px;" onclick="openAddPartModal()">+ 부품 추가</button></div></div>
      <table><thead><tr><th>부품명</th><th>현재 재고</th><th>최소 재고</th><th>상태</th><th style="width:130px;">관리</th></tr></thead><tbody id="partMgmtBody"></tbody></table>
    </div>
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem;">
      <div class="card"><h3>제품 등록 (부품 구성)</h3><div class="row"><input type="text" id="newProdName" placeholder="제품명" style="flex:1;"></div><div id="prodPartsList"></div><button class="secondary" style="font-size:13px;margin-bottom:8px;" onclick="addPartSlot()">+ 부품 추가</button><div><button class="secondary" onclick="saveProduct()">제품 저장</button></div></div>
      <div class="card"><h3>등록된 제품 목록</h3><table><thead><tr><th>제품명</th><th>구성 부품</th><th style="width:100px;">관리</th></tr></thead><tbody id="prodListBody"></tbody></table></div>
    </div>
    <div class="card">
      <h3>⚙️ 제품 선택 옵션 관리</h3>
      <p style="font-size:13px;color:#6b7280;margin-bottom:1.2rem;">출고 처리 화면의 계층 드롭다운 옵션을 편집합니다.</p>
      <div class="mgmt-sec"><h4>1단계 — 카테고리</h4><div class="tag-wrap" id="opt-cat"></div><div class="add-row"><input type="text" id="inp-cat" placeholder="예: 장롱"><button class="secondary small" onclick="addOptItem('categories','inp-cat')">추가</button></div></div>
      <div class="mgmt-sec"><h4>2단계 — 침대: 재질</h4><p class="note">이름에 "편백"이 포함되면 모델 없이 바로 사이즈로 이동합니다.</p><div class="tag-wrap" id="opt-pyun"></div><div class="add-row"><input type="text" id="inp-pyun" placeholder="예: 편백, 일반(원목)"><button class="secondary small" onclick="addOptItem('pyunbaek','inp-pyun')">추가</button></div></div>
      <div class="mgmt-sec"><h4>3단계 — 모델 (카테고리별)</h4>
        <p class="note">카테고리마다 모델을 따로 추가/삭제할 수 있습니다. 카테고리를 먼저 1단계에서 추가하세요.</p>
        <div id="opt-model-all"></div>
      </div>
      <div class="mgmt-sec">
        <h4>4단계 — 사이즈</h4>
        <p class="note">침대 기본 사이즈입니다. 소파/슈랑크 등 다른 카테고리는 아래 카테고리별 사이즈에서 따로 설정하세요.</p>
        <div class="tag-wrap" id="opt-size"></div>
        <div class="add-row"><input type="text" id="inp-size" placeholder="예: 싱글, 슈퍼싱글, 퀸, 킹"><button class="secondary small" onclick="addOptItem('sizes','inp-size')">추가</button></div>
      </div>
      <div class="mgmt-sec">
        <h4>4단계 — 카테고리별 사이즈 (선택)</h4>
        <p class="note">침대 외 카테고리에 사이즈가 있으면 여기서 설정하세요. 비워두면 사이즈 단계가 생략됩니다.</p>
        <div id="opt-cat-sizes"></div>
      </div>
      <div class="mgmt-sec"><h4>5단계 — 색상</h4><div class="tag-wrap" id="opt-color"></div><div class="add-row"><input type="text" id="inp-color" placeholder="예: 화이트, 월넛, 오크"><button class="secondary small" onclick="addOptItem('colors','inp-color')">추가</button></div></div>
      <div class="mgmt-sec"><h4>6단계 — 매트리스 선택 대상 모델</h4><p class="note">✅ 체크된 모델은 색상 선택 후 매트리스 여부 선택이 나타납니다.</p><div id="opt-matt-models"></div></div>
      <div class="mgmt-sec"><h4>7단계 — 매트리스 종류</h4><div class="tag-wrap" id="opt-matt"></div><div class="add-row"><input type="text" id="inp-matt" placeholder="예: 본넬스프링, 포켓스프링"><button class="secondary small" onclick="addOptItem('mattTypes','inp-matt')">추가</button></div></div>
      <div class="mgmt-sec"><h4>8단계 — 매트리스 사이즈</h4><div class="tag-wrap" id="opt-mattsize"></div><div class="add-row"><input type="text" id="inp-mattsize" placeholder="예: 싱글, 퀸, 킹"><button class="secondary small" onclick="addOptItem('mattSizes','inp-mattsize')">추가</button></div></div>
    </div>
  </div>
